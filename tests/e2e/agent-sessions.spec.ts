...
test("validates Cly Dev identity, workspace ownership, restoration, and fallback states", async ({
  ...
  for (const label of [
    ...
    await expect(
      group.getByRole("button", { name: `Show full ${label} identity` }),
    ).toBeVisible();
  ...
  for (const control of [
    ...
    page.getByRole("button", { name: "Inspect diff" }),
    page.getByLabel("Message the Orchestrator"),
  ]) {
    await expect(control).toBeVisible();
    const box = await control.boundingBox();
    expect((box?.x ?? 1025) + (box?.width ?? 0)).toBeLessThanOrEqual(1024);
    expect((box?.y ?? 701) + (box?.height ?? 0)).toBeLessThanOrEqual(700);
  }
