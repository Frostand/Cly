...
function seedSession(
  ...
  const outboundContext = normalizeDurableOutboundContext(
    repository.getOutboundContext("project-a", aggregate.session.id).egress,
  );
  repository.appendEvent(
    ...
    { outboundContext: outboundContext.envelope },
  );
  return aggregate;
}
