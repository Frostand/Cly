...
import {
  registerAgentConfigurationRoutes,
...
describe("agent configuration repository", () => {
  ...
  it("serves project-scoped CRUD, estimates, and revision conflicts", async () => {
    const { repository } = setup();
    const app = new Hono();
    registerAgentConfigurationRoutes(app, {
    ...
    const inaccessibleDraftResponse = await app.request(
      `${base}/draft/estimate`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
            ...
            roles: [
              {
                ...configurationInput.roles[0],
                model: "missing-model",
                fallbackModel: "missing-fallback",
                allowedTools: ["unknownTool"],
                allowedContextSources: ["unknown-context"],
              },
            ],
            maxParallel: 2,
          },
    ...
    const inaccessibleDraftEstimate = await inaccessibleDraftResponse.json();
    expect(inaccessibleDraftEstimate).toMatchObject({
      inaccessibleContext: ["unknown-context"],
      inaccessibleTools: ["unknownTool"],
      reasons: expect.arrayContaining([
        expect.stringMatching(/missing-model/),
        expect.stringMatching(/missing-fallback/),
        expect.stringMatching(/unknown-context/),
        expect.stringMatching(/unknownTool/),
      ]),
    });
    expect(inaccessibleDraftEstimate.reasons).toEqual(
      expect.arrayContaining([
        "Model “missing-model” is not available from provider “openai”.",
        "Fallback model “missing-fallback” is not available from provider “openai”.",
      ]),
    );
    ...
    const untrustedAvailabilityResponse = await app.request(
      `${base}/${created.id}/estimate`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ availableTools: ["writeFile"] }),
      },
    );
    expect(untrustedAvailabilityResponse.status).toBe(400);

    const discoveryFailureApp = new Hono();
    registerAgentConfigurationRoutes(discoveryFailureApp, {
      getRepository: () => repository,
      resolveAvailability: async () => ({
        contextSources: ["project"],
        providerCatalogs: {
    ...
    const unavailableResponse = await discoveryFailureApp.request(
      `${base}/${created.id}/estimate`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
    ...
    const unavailableEstimate = await unavailableResponse.json();
    expect(unavailableEstimate.reasons).toEqual(
      expect.arrayContaining([
        expect.stringMatching(
          /not installed.*Codex CLI.*could not be validated/i,
        ),
        expect.stringMatching(/Fallback model.*could not be validated/i),
      ]),
    );
    expect(unavailableEstimate.reasons).not.toEqual(
      expect.arrayContaining([
        expect.stringMatching(
          /Model .* is not available from provider “openai”/,
        ),
      ]),
    );
  });

  it("preserves provider discovery provenance and errors", async () => {
    const { db } = setup();
    const availability = await resolveAgentConfigurationAvailability({
      db,
      projectId: "project-1",
      discoverProviders: async () => ({
        ...
        opencode: Promise.reject(new Error("OpenCode discovery crashed.")),
        cursor: {
          installed: true,
          models: [{ id: "auto" }],
          source: "cli",
    ...
    expect(availability.providerCatalogs).toMatchObject({
      openai: {
        installed: false,
        models: [],
        source: "unavailable",
      ...
      opencode: {
        ...
        models: [],
        source: "error",
        error: "OpenCode discovery crashed.",
      },
    });
  });
});
