...
- Production startup composition: the renderer constructs and passes a fail-closed handoff dependency bundle into `startApiServer`; an authenticated startup test completes export, live Git/provider/policy inspection, and import against a migrated database and temporary committed repository.
- Database link enforcement: additive `0019_cly_dev_handoff_link_invariants.sql` uses insert/update triggers to require import direction, paired materialization fields, and a referenced session in the same project. Direct-SQL regressions cover orphan, cross-project, export-direction, half-populated, valid, reopened, already-recorded `0016`, and already-recorded `0018` states.
- Boundary hardening: materialized event keys use fixed-length SHA-256 suffixes instead of source ids, while progress labels are Unicode-safe and bounded to 500 code units without losing the exact current item in linked actionable state.
- Verification on 2026-07-16: focused production/migration/session/schema/renderer set `185/185`; full Vitest suite `546/546`; `pnpm typecheck` passed; full Biome `493` files passed.
