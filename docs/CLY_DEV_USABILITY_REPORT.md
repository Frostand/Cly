...
The exact tested product/source inputs are retained in `docs/CLY_DEV_TESTED_SOURCE_MANIFEST.sha256`, whose SHA-256 is `6099dad3235796cec71043b8e43f0cfb2d0e3091bcd67b839ba269611c690d4b`. The deterministic manifest records baseline `09fd7297fb11c7d7768f3c43de1b0cb8501fabe9` plus path and content SHA-256 for all 514 tracked source, Electron, E2E, public, dependency/workspace, build/test configuration, Electron preparation, capability-contract, package-hook, and accessibility-runner inputs used by the verified matrix. Evidence documents, generated output, the manifest, and its verifier are deliberately excluded so the artifact is reproducible without self-reference. `node scripts/verify-cly74-tested-source-manifest.mjs` verifies both scope and content. The matrix reran at 2026-07-16T17:41:01Z on macOS with Node 24.18.0 and Electron 41.7.2.

The screenshot-based heuristic assessment is historical and explicitly tied to the pre-correction build; the current automated matrix above is the evidence for the corrected CSS.
